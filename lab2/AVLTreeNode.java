 ...
 * @author Magnus Nielsen (magnus.nielsen@liu.se)
 * Partially based on C++-laborations by Tommy Olsson and Filip Strömbäck.
 */
public class AVLTreeNode<T extends Comparable<T>> {
     ...
     *
     * @param data - Generic parameter containing the data element for the node.
     * @param l - AVLTreeNode<T> containing the intended left child for the node being created.
     * @param r - AVLTreeNode<T> containing the intended right child for the node being created.
     */
    ...
    /**
     * Insert element into the tree originating from t. Currently does
     * not balance the tree.  
    ...
    //Check recursively parents balance
    if(nodeHeight(right) - nodeHeight(left) > 1 ){ // right - men nästa left or right
        if(nodeHeight(right.right)-nodeHeight(right.left) > 0){ // right - right
            return singleRotationWithRightChild(this); // tar bort men inte roterar
        } else {
            return doubleRotationWithRightChild(this);
        }
    } else if(nodeHeight(left) - nodeHeight(right) > 1){ //left 
        if(nodeHeight(left.left) - nodeHeight(left.right) > 0){ // left - left
            return singleRotationWithLeftChild(this); // tar bort men inte roterar
        } else { // left - right
    ...
    /**
     * Search for a node containing the key. If found: return the node, otherwise return null.
     *
    ...
    /**
     * Looks for the node with the smallest value in the tree (the leftmost node), and returns that node.
     * If the tree is empty, null will be returned.
     *
     * @return AVLTreeNode with the smallest value in the tree, null if the tree is empty.
     */
    ...
    /**
     * Looks for the node with the largest value in the tree (the rightmost node), and returns that node.
     * If the tree is empty, null will be returned.
     *
     * @return AVLTreeNode with the largest value in the tree, null if the tree is empty.
     */
    ...
    /**
     * Removes the node key from the tree if it exists. Currently
     * removes in a lazy fashion (does not balance the tree).
     * TODO: Implement balancing.
     *
     * @param key - Generic parameter with the key to be deleted.
     * @return - AVLTreeNode<T>, the correct node for the source position in the tree.
     */
