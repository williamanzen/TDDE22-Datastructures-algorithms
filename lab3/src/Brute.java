...
public class Brute {
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        f = new File("C:\\Users\\willi\\Documents\\tdde22\\lab3\\src\\data\\grid4x4.txt");
        try {
            input = new Scanner(f);
        ...
        for (int i = 0; i < count - 3; ++i) { // 0..n-3
	    for (int j = i + 1; j < count - 2; ++j) {	// 1..n-2
		for (int k= j + 1; k < count - 1; ++k) { // 2..n-1
		    if (points.get(i).slopeTo(points.get(j)) == points.get(i).slopeTo(points.get(k))) {
                        for (int m = k + 1; m < count; ++m) { // 3..n 
